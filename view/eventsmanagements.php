...
WHERE e.OrganizerID = ? AND e.EndTime >= NOW() AND e.EventStatusID = 1
...
<div class="navbar">
    ...
    <div class="nav-items">
        <a href="#" class="active">Managing My Events</a>
            ...
            <div class="event-card" data-event-title="<?php echo htmlspecialchars($event['Title']); ?>">
                ...
                <div class="card-content">
                    ...
                    <div class="event-actions">
                        ...
                        <!-- Edit button -->
                        <button type="button" class="btn edit-btn" onclick="openEditModal('<?php echo $event['EventID']; ?>', '<?php echo addslashes($event['Title']); ?>', '<?php echo addslashes($event['Description']); ?>', '<?php echo $event['StartTime']; ?>', '<?php echo $event['EndTime']; ?>', '<?php echo addslashes($event['VenueName']); ?>', '<?php echo addslashes($event['VenueLocation']); ?>', '<?php echo $event['VenueCapacity']; ?>', '<?php echo addslashes($event['VenueContact']); ?>','<?php echo htmlspecialchars($event['ImagePath']); ?>')">Edit</button>

                        <!-- Delete button -->
                        <a href="../action/deleteeventaction.php?event_id=<?php echo $event['EventID']; ?>" class="btn delete-btn" onclick="return confirm('Attention: Deleting this event will trigger refunds for all paid tickets. As the Super Admin holds the payments, you will not receive any revenue from this event once it is deleted. Are you sure you want to proceed with deleting this event?');">Delete</a>
                       
                       <!-- Manage Tickets & Seats button -->
                       <button type="button" class="btn manage-tickets-btn" onclick='openModal(<?php echo $event["EventID"]; ?>, `<?php echo addslashes($event["Title"]); ?>`, <?php echo json_encode($event["TicketTiers"]); ?>)'>Manage Tickets & Seats</button>

                       <a href="seeAttendees.php?event_id=<?php echo $event['EventID']; ?>" class="btn view-attendees-btn">Check-in attendees</a>
...
<div id="ticketsModal" class="modal" style="display: none;">
  <div class="modal-content">
    ...
    <h2>Manage Ticket Tiers for Event:(You can Always Edit Existing Ones, In case of promotions..) <span id="modalEventTitle"></span></h2>
